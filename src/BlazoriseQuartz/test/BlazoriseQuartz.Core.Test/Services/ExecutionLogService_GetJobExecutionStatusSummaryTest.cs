...
namespace BlazoriseQuartz.Core.Test.Services
{
    public class ExecutionLogService_GetJobExecutionStatusSummaryTest
    {
        private ExecutionLogService sut;
        private BlazoriseQuartzDbContext context;

        public ExecutionLogService_GetJobExecutionStatusSummaryTest()
        {
            context = CoreTestUtils.GetInMemoryBlazoriseQuartzDbContext(Guid.NewGuid().ToString());
            var contextFactoryMock = new Mock<IDbContextFactory<BlazoriseQuartzDbContext>>();
